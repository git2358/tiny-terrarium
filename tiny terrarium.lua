...
function iterate(p,x,y)
	...
	elseif p=='bug' then
		...
		if r==0
		and grid:is_in_bounds(x+rx,y+ry)
		and grid(x+rx,y+ry)~="block" then
			order(x,y,x+rx,y+ry)
		end
	end
end
...
function new_grid(w,h)
	...
	function r.set(_,x,y,v)
		es[xy_to_s(x,y)]=v
		sset(x,y+64,acol[v])
	end
	function r.swap(_,x1,y1,x2,y2)
		local v1=r(x1,y1)
		local v2=r(x2,y2)
		r:set(x1,y1,v2)
		r:set(x2,y2,v1)
	end
	function r.is_in_bounds(_,x,y)
		return mid(0,x,w-1)==x
		   and mid(0,y,h-1)==y
	end
	function r.is_air(_,x,y)
		return r:is_in_bounds(x,y)
		   and r(x,y)==nil
	end
	return r
end
